...
// Phase 2: stronger feature stack, better price handling, confident closer tone.
...
function cleanPrice(raw) {
  ...
  // Best: line that clearly looks like a money value
  const dollarLine = lines.find(l => /\$\s*\d/.test(l));
  if (dollarLine) return dollarLine;

  // Next: any numeric line that isn't just the word "Price"
...
// ---------- FEATURE STACK (Phase 2: smarter, more “equipment” feel) ----------
...
function generateHashtags(vehicle) {
  ...
  // Special combined tags for common patterns
