...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // calculators
  const calcPayForm = document.getElementById("calcPayForm");
  const calcPayPrice = document.getElementById("calcPayPrice");
  const calcPayDown = document.getElementById("calcPayDown");
  const calcPayTerm = document.getElementById("calcPayTerm");
  const calcPayApr = document.getElementById("calcPayApr");
  const calcPayResult = document.getElementById("calcPayResult");
  const calcPayFinanced = document.getElementById("calcPayFinanced");
  const calcPayTotal = document.getElementById("calcPayTotal");
  ...
  const calcIncomeClose = document.getElementById("calcIncomeClose");
  const calcIncomeDealsMonth = document.getElementById("calcIncomeDealsMonth");
  const calcIncomeDealsDay = document.getElementById("calcIncomeDealsDay");
  const calcIncomeShowsDay = document.getElementById("calcIncomeShowsDay");
  const calcIncomeApptsDay = document.getElementById(
    "calcIncomeApptsDay"
  );
  ...
  function renderPhotosGrid(photos) {
    ...
    photosStatus.textContent =
      photos.length +
      " photos found. Click any to open full size.";
  }
  ...
  async function handleBoost() {
    ...
    try {
      const resp = await callJson("/api/social-kit", {
        url,
        label,
        price,
      });
  ...
  buildVideoButton.addEventListener("click", async () => {
    ...
    try {
      const label =
        safeTrim(vehicleLabelInput.value) || "this vehicle";
  ...
  // payment calculator
  calcPayForm.addEventListener("submit", (e) => {
    e.preventDefault();
    const price = Number(calcPayPrice.value) || 0;
    const down = Number(calcPayDown.value) || 0;
    const term = Number(calcPayTerm.value) || 0;
    const apr = Number(calcPayApr.value) || 0;

    if (!price || !term) {
      calcPayResult.textContent = "$0.00 / mo";
      calcPayFinanced.textContent = "$0.00";
      calcPayTotal.textContent = "$0.00";
      return;
    }
    ...
    calcPayResult.textContent = formatMoney(payment) + " / mo";
    calcPayFinanced.textContent = formatMoney(amountFinanced);
    calcPayTotal.textContent = formatMoney(totalPaid);
  });
  ...
  calcAffordForm.addEventListener("submit", (e) => {
    ...
    calcAffordPrice.textContent = formatMoney(maxPrice);
  });
  ...
  calcIncomeForm.addEventListener("submit", (e) => {
    ...
    const perDeal = Number(calcIncomePerDeal.value) || 0;
    const days = Number(calcIncomeDays.value) || 0;
    const closeRate = Number(calcIncomeClose.value) || 0;

    if (!goal || !perDeal || !days || !closeRate) {
      calcIncomeDealsMonth.textContent = "0.0";
      calcIncomeDealsDay.textContent = "0.00";
      calcIncomeShowsDay.textContent = "0.00";
      calcIncomeApptsDay.textContent = "0.00";
      return;
    }
    ...
    const showsPerDay =
      closeRate > 0 ? dealsPerDay / (closeRate / 100) : 0;
    const apptsPerDay = showsPerDay * 1.2; // simple fudge factor

    calcIncomeDealsMonth.textContent = dealsPerMonth.toFixed(1);
    calcIncomeDealsDay.textContent = dealsPerDay.toFixed(2);
    calcIncomeShowsDay.textContent = showsPerDay.toFixed(2);
    calcIncomeApptsDay.textContent = apptsPerDay.toFixed(2);
  });
});
